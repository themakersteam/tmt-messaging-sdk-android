package com.tmt.messagecenter.connection_xmpp;

import android.content.Context;
import com.tmt.messagecenter.connection_xmpp.constants.ChatErrorCodes;
...
import com.tmt.messagecenter.connection_xmpp.extensions.SeenReceiptExtension;
import com.tmt.messagecenter.connection_xmpp.interfaces.SendFileInterface;
import com.tmt.messagecenter.connection_xmpp.interfaces.XmppChatCallbacks;
import com.tmt.messagecenter.connection_xmpp.interfaces.XmppClientInterface;
import com.tmt.messagecenter.connection_xmpp.manegers.S3FileUploadManager;
...
public class XmppClient implements XmppClientInterface {
    ...
    public void onCreate(Context context, final XmppChatCallbacks chatCallbacks, String chatId) {
        ...
        if (chatCallbacks != null) {
            ...
            if (MessageCenter.instance().getConnection() == null)
    ...
    public void sendFileMessage(Context context, File file, String caption, SendFileInterface sendFileInterface) {
        try {
            S3FileUploadManager.getInstanceFor(MessageCenter.instance().getConnection()).sendFile(context, chat, file, caption, sendFileInterface);
        }
        catch (Exception e){
    ...
    public List<UserMessage> loadPrevMessages(int page_limit) {
        try {
            MamManager mamManager = MamManager.getInstanceFor(MessageCenter.instance().getConnection(), chat.getRoom());
    ...
    private void setupResourcePart() {
        try {
            resourcepart = Resourcepart.from(MessageCenter.instance().getConnection().getUser().asEntityBareJid().getLocalpart().toString());
        }
        catch (Exception e){}
    }

    private void registerStanzaFilter() {
        MessageCenter.instance().getConnection().addAsyncStanzaListener(new StanzaListener() {
            @Override
    ...
    private void registerConnectionListener() {
        ReconnectionManager.getInstanceFor(MessageCenter.instance().getConnection()).addReconnectionListener(new ReconnectionListener() {
            @Override
        ...
        MessageCenter.instance().getConnection().addConnectionListener(new ConnectionListener() {
            @Override
    ...
    private void joinRoom(String chat_id) {
        try {
            ...
            chat = MultiUserChatManager.getInstanceFor(MessageCenter.instance().getConnection()).getMultiUserChat(jid);
            MultiUserChatManager.getInstanceFor(MessageCenter.instance().getConnection()).setAutoJoinOnReconnect(true);
            try {
